...
#define         MQ2_SENSOR                   (4)  //define which analog input channel you are going to use
#define         MQ6_SENSOR                   (1)
...
unsigned long SLEEP_TIME = 1; // Sleep time between reads (in seconds)
//VARIABLES
...
void setup()
{
  ...
  Ro0 = MQCalibration(MQ135_SENSOR, 0.5, RL0, CO2Curve);
...
{
  long val = (long)((long)(1024 * 1000 * (long)rl_value) / raw_adc - (long)rl_value);
  return val;
}
